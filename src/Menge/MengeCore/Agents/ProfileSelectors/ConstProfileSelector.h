...
/*!
 ...
 Simply exchange the `PROFILE_NAME` for a named profile in the scene specification file.
 */
class MENGE_API ConstProfileSelector : public ProfileSelector {
 public:
  ...
  /*!
   ...
   Although the ProfileSelector can cache pointers to the profiles for internal convenience, it
   does not own the memory and should not delete them.
...
class MENGE_API ConstProfileSelectorFactory : public ProfileSelectorFactory {
 public:
  ...
  /*!
   ...
   The profile selector's name must be unique among all registered profile selector elements. Each
   profile selector factory must override this function.
 ...
 protected:
  /*!
   ...
   All ProfileSelectorFactory sub-classes must override this by creating (on the heap) a new
   instance of its corresponding selector type. The various field values of the instance will be
   set in a subsequent call to ProfileSelectorFactory::setFromXML. The caller of this function
  ...
  /*!
   @brief		Given a pointer to a pProfileSelector instance, sets the appropriate fields from the
            provided XML node.

   It is assumed that the value of the `type` attribute is this ProfileSelector's type (i.e.
   ProfileSelectorFactory::thisFactory has already been called and returned true). If sub-classes
   of ProfileSelectorFactory introduce *new* ProfileSelector parameters, then the sub-class should
