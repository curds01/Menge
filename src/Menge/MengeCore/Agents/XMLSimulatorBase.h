...
/*!
 ...
 @brief     The set of operations used by SimXMLLoader to apply XML-parsed experiment specification
            to a simulator.

 In order to use the SimXMLLoader class to read XML files and set parameters. The simulator must
 extend this abstract class and implement the given virtual functions.
...
#ifdef _MSC_VER
// This is visual studio non-ansi compliant functionality. It doesnt like functions which declare
// specific types of exceptions thrown. This turns off the silly visual studio warning.
...
/*!
 @brief		Exception thrown when the xml parameter parser has an error which cannot be recovered
          from.
 */
...
/*!
 @brief		The base class for extracting simulator settings from the XML specification.

 In order to use the SimXMLLoader class to read XML files and set parameters, the simulator must
 extend this abstract class and implement the given virtual functions.

 A "target"-specific parameter refers to the simulator type. In other words, if a simulator expects
 particular parameters, there should be a tag with the target name and a list of parameter values.
 The functionality of this class defines that functionality.
 */
class MENGE_API XMLSimulatorBase {
 public:
  ...
  /*!
   ...
   @returns	True if this simulator has target-specific *experiment* parameters, false otherwise.
   */
  virtual bool hasExpTarget() { return true; }

  /*!
   @brief		Given the name of a tag which is the child of `<Experiment>`, reports if it contains
            target-specific experiment parameters.
   ...
   @returns	True if the given tag corresponds to the unique tag defined for this simulator (and
            that tag's parameters should be applied to this simulator).
   */
  virtual bool isExpTarget(const std::string& tagName) = 0;

  /*!
   @brief		Given the name of the xml param and its string value, sets the parameter in the
            simulator.

   Common parameters and target-specific parameters will be passed into this function. It is the
   responsibility of the programmer to make sure the default case of any parameter should fall
   through to the SimulatorBase::setExpParam function.

   @param		paramName		The name of the XML parameter.
   @param		value			  The string representation of the parameter value. It is the responsibility
                        of the particular sub-class to translate the representation as necessary.
