...
class MENGE_API PortalPath {
 public:
   ...
   *be
   *								computed.
   *	@param		headingCos		The cosine of the maximum allowable angular
   *deviation
   *								of heading (between planned and realized) before
   *a
   *								new funnel algorithm is triggered to improve the
   *path.
   *	@param		pVel			The preferred velocity whose directions are to be
   *set.
   */
   ...
   *words,
   *							portals in the open interval [start, end) will
   *receive
   *							crossing values.  end will not be included.
   *	@param		p0			The way point.
   *	@param		dir			The expected direction toward the way point along the
   *path.
   */
