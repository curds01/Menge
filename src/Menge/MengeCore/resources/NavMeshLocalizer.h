...
class MENGE_API NavMeshLocalizer : public Resource {
  ...
  *							point.
  *	@param		searchAll	If true and no polygon in `grpName` contains the point,
  *search
 ...
 protected:
   ...
   *								value leads to the highest node being
   *selected.
   *								But giving an alternative value will change it
   *to
   *								select a node nearest the current
   *elevation.
   ...
   *							will cause the algorithm to find an alternative
   *anywhere
   *							in the mesh.
