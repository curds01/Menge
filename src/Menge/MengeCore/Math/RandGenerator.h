...
class MENGE_API NormalFloatGenerator : public FloatGenerator {
 public:
   ...
   *							be greater than or equal to minVal.  For largely
   *"pure"
   *							normal distribution, minVal <= mean - 3 *
   *stddev.
   *	@param		maxVal		The upper clamped value, such that all values returned will
   *							be less than or equal to maxVal.  For largely "pure"
   *normal
 ...
 *	@param		prefix		An optional prefix which may be affixed to the float
 *generator
 *							keywords.
