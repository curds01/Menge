...
class MENGEVIS_API ContextResult {
 public:
   ...
   *handler.
   *	@param		needsRedraw		Set to true if the processing of the event requires
   *a
   *								redraw.
   */
 ...
 protected:
  /*!
   *	@brief		Reports if the event has been handled (and no one else needs to worry
   *				about it).
   */
  bool _handled;
...
class MENGEVIS_API Context {
 ...
 protected:
   ...
   *	@param		pos			The position, in world coordinates, to place the
   *text
   ...
   *	@param		pos			The position, in world coordinates, to place the
   *text
   *	@param		dir			A vector representing the direction of the radius at
   *whose
   *							end the text should be placed.  Direction defined
   *in
   *							world coordinates
   *	@param		currColor	Indicates if the current opengl color should be used (true)
   *							or the text writer's default color should be used
...
/*!
 *	@brief		A context that performs object selection by directly clicking on selectable
 *				entities in the OpenGL context.
 */
