...
class MENGEVIS_API TextWriter {
 ...
 public:
   ...
   *							or the text writer's default color should be used
   *(false).
   *	@param		transparent	Indicates if the text is super-imposed over the screen or
   *							given amatte.  True --> superimposed, false -->
   *matted.
   */
   ...
   *							coords).
   *	@param		bottom		The position of the bottom extent of the text (in screen
   *							coords).
   *	@param		fontSize	The size of the text, in pixels.
   *	@param		currColor	Indicates if the current opengl color should be used (true)
   *							or the text writer's default color should be used
   *(false).
   *	@param		transparent	Indicates if the text is super-imposed over the screen or
   *							given a matte.  True --> superimposed, false -->
   *matted.
   */
   ...
   *	@param		alignment	The alignment, relative to the screen.
   *	@param		fontSize	The size of the text, in pixels.
   *	@param		hpad		The amount of horizontal padding to apply in the horizontal
   *							alignment direction.  For example, if left aligned, it
   *goes
   *							on the left, if right aligned, the padding is on the
   *right.
   *	@param		vpad		The amount of vertical padding to apply in the vertical
   *							direction.
   *	@param		currColor	Indicates if the current opengl color should be used (true)
   *							or the text writer's default color should be used
   *(false).
  ...
  /*!
   *	@brief		Prints the text anchored to the position.  The relationship to the
   *				anchor depends on the anchor weights.  It interpolates them from
   *bottom
   *				left top top right.
   *	@param		text		The string to draw.
   *	@param		anchorX		The x-position of the anchor point (in screen coords).
   *	@param		anchorY		The y-position of the anchor point (in screen coords).
   *	@param		xWeight		The horizontal weight in the range [0, 1]. 0--> left, 1 -->
   *							right alignment.
   *	@param		yWeight		The vertical weight in the range [0, 1]. 0--> bottom, 1 -->
   *							top alignment.
   *	@param		fontSize	The size of the text, in pixels.
   *	@param		currColor	Indicates if the current opengl color should be used (true)
   *							or the text writer's default color should be used
 ...
 protected:
  ...
  /*!
   *	@brief		The width of the viewport the TextWriter considers when computing text
   *				positions.
   */
  int _width;

  /*!
   *	@brief		The height of the viewport the TextWriter considers when computing text
   *				positions.
   */
  int _height;
   ...
   *	@param		lineNum		The line number of the given text (zero-indexed).  Used to
   *							offset this line relative to the anchor
   *point.
   */
