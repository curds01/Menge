...
// Now set up compiler-dependent export/import symbols
  ...
  #if defined( MIXED_STATICLIB )
    #define MIXED_API
  #else
    #if defined(  MIXED_EXPORT )
      // We are building the DLL, export the symbols tagged like this
      #define MIXED_API __declspec(dllexport)
    #else
      // If we are consuming the DLL, import the symbols tagged like this
      #define MIXED_API __declspec(dllimport)
    #endif
  #endif
  ...
  #else
    #if defined( MIXED_EXPORT )
      #define MIXED_API __attribute__ ((visibility ("default")))
    #else
