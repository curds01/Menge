...
class EXPORT_API HeightField : public Menge::Resource {
 public:
   ...
   *	@brief		Constructor.
   *
   *	@param		fileName		The height field specification file used to define the height
   *field.
   */
 ...
 public:
   ...
   *				"minimum" corner at the origin and extending into the first quadrant
   *of
   *				the x-z plane based on cell size and image resolution.
   *
   *	@param		imgName			The image file used to define the height field.
   *	@param		cellSize		The size of each cell in the image in world
   *coordinates.
   *	@param		vertScale		The values of the image (in the range [0, 255] are
   *								normalized to the range [0, 1] and then multiplied by
   *this
   *								vertical scale.
   *  @param		xpos			The x-coordinate of the terrain's origin
   *	@param		zpos			The z-coordinate of the terrain's origin
   *	@param		smoothParam		The smoothing parameter for the elevation values.  It
   *is
   *								interpreted as the standard deviation of a 2D
   *symmetric
   *								gaussian kernel.  If zero, no smoothing will be
   *performed.
   ...
   *				outside of the domain of the height field, the height at the nearest
   *cell
   *				center is returned.
   *
   ...
   *				position lies outside of the domain of the height field, the normal of
   *the
   ...
   *				The behavior is undefined if the indices fall outside the array of cell
   *values.
   *
