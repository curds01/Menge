 ...
 *	@brief		definition of a VelocityModifier to push agents along the gradient of a
 *heightField
 */
 ...
 *	preferred direction.
 *
 *	The resultant speed is a scaled factor of the input speed with the steepness of the
 *gradient.
 *	Uphill and downhill speed changes are controlled separately -- gradients which lie
 *perpindicular
 ...
 *		prevents the speed from changing at all.  A value of 1.0 will allow the speed to
 *double).
 *	- The value `dir_weight` is a scale factor which determines how much the travel direction is
 *		affected by the height field.  Smaller values will cause smaller changes to direction
 *than
 *		larger values.  (A value of zero will leave the agent moving along their original
 *direction,
 *		but still susceptible to speed changes.)
 *	- The value `up_hill_scale` is a scale factor applied to up hill motion.
 *		It limits the amount that the speed is retarded when moving up hill.  If the value is
 *1,
 *		then only a vertical surface will *completely* eliminate motion.  However, larger
 *values
 *		wall cause the agent to slow to a stop on smaller slopes.  Smaller values will cause
 *the
...
class EXPORT_API HeightFieldModifier : public Menge::BFSM::VelModifier {
 public:
   ...
   *	@param		pVel		The agent's current preferred velocity (having potentially
   *been
   *							modified)
   *
   */
