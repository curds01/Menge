 ...
 *
 *	This namespace contains a velocity modifier which varies preferred speed based on local
 *density
 *	conditions.
 */
namespace FDModifier {
 ...
 *		be replaced by the pairs `buffer_min` and `buffer_max` or `buffer_mean` and
 *`buffer_stddev`,
 *		respectively.
 ...
 *		be replaced by the pairs `factor_min` and `factor_max` or `factor_mean` and
 *`factor_stddev`,
 *		respectively.
 */
